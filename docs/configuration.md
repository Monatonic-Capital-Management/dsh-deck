...
选定路径不存在时，在内存中使用仅含 `local` 的默认配置；不会退回另一配置，也不因 `list` 或 `plan` 创建配置文件。登记或编辑时会保存；面板启动流程发现该文件不存在时，也会保存默认配置。写入使用同目录临时文件和原子替换。默认本地工作目录是用户目录；显式指定的目录不存在会拒绝启动，不静默换目录。
