...
[返回首页](../../README.md) · [当前贡献指南](../../CONTRIBUTING.md) · [实施计划](../implementation-plan.md)

> 历史贡献指南，归档于 2026-09-16。保留过去的测试经验；原始诊断附件建议和历史 smoke test 不能直接用于当前工作环境。当前要求见[安全与隐私](../security.md)。
>
> **DOC-PACKAGE / DOC-CHECKS｜本轮实施目标。** 基线 EXE 清单遗漏远端脚本且按时间戳判断普通重建；完整内容清单、资源一致性和离线文档门禁待实现。本文剩余精炼及最终检查命令由主代理接续，未运行的检查不得记为通过。
...
rather than trusting that a command exited 0. See [docs/lessons.md](2026-09-16-lessons.md)
