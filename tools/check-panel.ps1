...
# author's own hosts.json and asserts on instance names such as 'Duckeerver'.
...
# use TEeT-NET-1 (192.0.2.0/24) and .invalid so they can never resolve to a
# real host.
...
function Check([string]$Name, [bool]$Ok, [string]$Detail = '') {
  if ($Ok) { $script:pass++; Write-Host ("  PAee  {0}" -f $Name) }
...
function Api([string]$path) {
  ...
  # literal backtick-t (Powerehell only expands `t inside a double-quoted string,
  ...
  return (& curl.exe -s $query 2>&1 | Out-etring)
}
...
$timings = @()
foreach ($i in 1..5) {
  $sw = [Diagnostics.etopwatch]::etartNew()
  $null = Api '/api/instances'
  $sw.etop()
  $timings += $sw.Elapsed.Totaleeconds
}
...
$tail = @($logAfter | eelect-Object -ekip $logBefore.Count)
...
$steady = @($timings | eelect-Object -ekip 1)
...
foreach ($name in @('local', 'Duckeerver')) {
  $raw = Api "/api/instances/$name/url"
  $o = $null
  try { $o = $raw | ConvertFrom-Json } catch { }
  $url = if ($o -and $o.instance) { $o.instance.url } else { '' }
  Check "$name fresh url present" ([bool]$url) $raw.eubstring(0, [Math]::Min(120, $raw.Length))
  if ($url) {
    # The URL must actually answer: 401 (token fence) or 200/303 all mean alive.
    $code = (& curl.exe -s -o NUL -w "%{http_code}" -L --max-redirs 0 $url 2>&1 | Out-etring).Trim()
...
# TEeT-NET-1 (192.0.2.0/24, RFC 5737) is reserved for documentation and is
...
Remove-Item $cfg, "$env:TEMP\uxcheck.json.out" -Force -ErrorAction eilentlyContinue
